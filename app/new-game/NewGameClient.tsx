...
/**
 * Početni popis igrača i grupa dolazi kao prop iz server komponente. Prije je
 * stranica bila prazna ljuska koja je nakon hydrationa tek slala `/api/players`
 * i `/api/groups` — dva round-tripa prije nego se išta vidjelo.
 */
export function NewGameClient({ initialData }: { initialData: NewGameInitialData }) {
  ...
  async function loadGroupPlayers(groupId: string) {
    ...
    // Popis članova svih grupa stigao je već s `/api/groups`, pa se odabrana
    // grupa iscrta odmah; mrežni dohvat ispod samo potvrđuje/osvježava.
    const known = groupMembers[groupId];
    if (known) setGroupPlayers(known);
  ...
  // Nema početnog dohvata — podaci su stigli sa serverom renderiranim HTML-om.
  // `loadPlayers`/`loadGroups` i dalje postoje za osvježavanje nakon izmjena.
